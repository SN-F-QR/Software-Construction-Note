...
const anyscale = new GoogleGenerativeAI(ANYSCALE_API_KEY);
const model = anyscale.getGenerativeModel({ model: "gemini-1.5-flash" });
const embedModel = anyscale.getGenerativeModel({ model: "text-embedding-004" });
...
const validateAPIKey = async () => {
  try {
    ...
    // });
    await model.generateContent("Could you response");
    hasapikey = true;
    return hasapikey;
  } catch {
...
const anyscaleEmbedding = async (document) => {
...
const googleEmbedding = async (document) => {
  const result = await embedModel.embedContent(document);
  return result.embedding.values;
};

const generateEmbedding = googleEmbedding;
...
const chatCompletion = async (query, context) => {
  ...
  const completion = await chat.sendMessage(query);
